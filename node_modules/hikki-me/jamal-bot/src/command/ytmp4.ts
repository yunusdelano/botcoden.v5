...
export let handler: ICommand = async (m: IMessage, { args, jamal, command }): Promise<any> => {
  ...
  let text = `*(📌 YOUTUBE ${data.type.toUpperCase()} 📌)*\n\n`
  text += `*🔖 Title:* ${data.title}\n`
  text += `*🔖 Size:* ${data.fileSize}\n`
  text += `*🔖 Quality:* ${data.quality}\n`
  text += `*🔖 Mimetype:* ${data.type}\n`
  if (isSize) text += `*🔖 Link:* ${data.url}\n\n`
  if (isSize) text += `*📌 ${command.includes('mp3' || 'music') ? 'Music' : 'Video'} Melebihi ${isLimit}MB, Mohon Unduh Secara Manual*\n`
  else text += `*📌 ${command.includes('mp3' || 'music') ? 'Music' : 'Video'} Akan Dikirim Dalam Beberapa Detik, Mohon Tunggu...*\n`
  jamal.sendFile(m.chat, data.thumbnail, { caption: text })
  if (!isSize && isLimit == 100) jamal.sendTButton(m.chat, true, '', 'Klik Jika Ingin Mengambil Audio Dari Video', ['AUDIO 🎵', '.ytmp3 ' + args.join(' ')], null, data.url)
