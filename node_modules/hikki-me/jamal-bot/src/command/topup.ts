...
export let handler: ICommand = async (m: IMessage, { args, jamal, command }): Promise<any> => {
  if (command == 'gotopup') {
    ...
    let menu = `*( 🔖 TOP UP 🔖 )*\n\n`
    menu += `*🔖 Nama:* ${json.data.userNameGame}\n`
    menu += `*🔖 Nominal:* ${json.data.item.name}\n`
    menu += `*🔖 Payment:* ${'QRIS'}\n`
    menu += `*🔖 Silahkan Scan QR Code:*\n`
    ...
    jamal.sendTWithUrlButton(m.chat, true, menu, getUser.data.userNameGame, ['Website', 'https://hardianto.xyz', 'Setuju ☑', `#gotopup ${jumlah} ${args[0]}`], null, 'https://file.hikki.me/uploads/1k17463rqs.mp4')
  }
}
